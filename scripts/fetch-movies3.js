/**
 * Fetches TMDB data for movies in movies3.txt and merges into public/curated.json.
 * Skips movies already in curated.json (by TMDB ID) and movies without poster_path.
 * Usage: node scripts/fetch-movies3.js
 */
import fs from 'fs';
...
const MOVIES_FILE   = path.resolve('C:/Users/eito1/movie_txt/movies3.txt');
const CURATED_FILE  = path.resolve(__dirname, '..', 'public', 'curated.json');
const PROGRESS_FILE = path.resolve(__dirname, '..', '.movies3-progress.json');
...
async function main() {
  ...
  console.log(`Total entries in movies3.txt: ${entries.length}`);
